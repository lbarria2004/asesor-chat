...
export async function POST(request: NextRequest) {
  try {
    ...
    conversationHistory.push({
      role: 'user',
      parts: [{ text: `INSTRUCCIONES DEL SISTEMA - DEBES SEGUIR ESTAS REGLAS:\n\n${pensionSystemPrompt}` }]
    });

    // Add model acknowledgment
    conversationHistory.push({
      role: 'model',
      parts: [{ text: 'Entendido. Soy el Asesor Previsional Virtual de Luis Barría Chodil. Seguiré todas las reglas: responderé de forma concisa (3-4 oraciones máximo), siempre terminaré con una pregunta de seguimiento, y después de 2-3 intercambios ofreceré asesoría gratuita. ¿En qué puedo ayudarte?' }]
    });
